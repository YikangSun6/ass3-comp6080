...
export const assertValidUserId = (userId) =>
...
export const assertValidJobPost = (jobPostId) =>
  dataLock((resolve, reject) => {
...
export const assertCreatorOfJobPost = (userId, jobPostId) => {
  if (posts[jobPostId].creatorId !== parseInt(userId, 10)) {
    throw new AccessError(
      "Authorised user is not the creator of this job post"
    );
  }
};
...
export const getJobs = (authUserId, start) =>
  dataLock((resolve, reject) => {
    ...
    const relevantPosts = allPosts.filter((p) =>
      Object.keys(users[p.creatorId].watcheeUserIds).includes(authUserId)
    );
...
export const postJob = (authUserId, image, title, start, description) =>
  dataLock((resolve, reject) => {
    if (
      image === undefined ||
      title === undefined ||
...
export const commentOnJobPost = (authUserId, jobPostId, comment) =>
  dataLock((resolve, reject) => {
...
export const likeJobPost = (authUserId, jobPostId, turnon) =>
  dataLock((resolve, reject) => {
    if (turnon) {
...
export const deleteJobPost = (authUserId, jobPostId) =>
  dataLock((resolve, reject) => {
    delete posts[jobPostId];
    resolve();
  });
export const addComments = (Id,userId,comment) =>
  dataLock((resolve, reject) => {
    console.log("posts[Id]",posts[Id],Id)
    posts[Id].comments.push({userId,comment});
    resolve();
  });
  export const deleteComments = (index, id,Id) =>
  dataLock((resolve, reject) => {
    //delete posts[Id].comments[index];
    posts[Id].comments.splice(index,1)
    resolve();
  });
...
export const getUser = (userId) =>
  dataLock((resolve, reject) => {
    ...
    const userDetails = {
      ...
      password: undefined,
      id: intid,
      watcheeUserIds: Object.keys(users[userId].watcheeUserIds).map((i) =>
        parseInt(i, 10)
      ),
      jobs: Object.keys(posts)
        .map((pid) => posts[pid])
        .filter((post) => post.creatorId === intid)
        .map(buildJobObject),
    };
    resolve(userDetails);
  });
...
export const getusers = () =>
  dataLock((resolve, reject) => {
    ...
    for (var key in users) {
      if (key != "id") {
        const intid = parseInt(key, 10);
        arr.push({
          ...users[key],
          id: key,
          watcheeUserIds: Object.keys(users[key].watcheeUserIds).map((i) =>
            parseInt(i, 10)
          ),
          jobs: Object.keys(posts)
            .map((pid) => posts[pid])
...
export const watchUser = (watcherUserId, watcheeUserId, turnon) =>
  dataLock((resolve, reject) => {
    if (turnon === undefined) {
...
export const updateProfile = (authUserId, email, password, name, image) =>
  dataLock((resolve, reject) => {
    if (name) {
      users[authUserId].name = name;
    }
    if (password) {
      users[authUserId].password = password;
    }
    if (image) {
      users[authUserId].image = image;
    }
